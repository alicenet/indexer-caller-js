...
describe("IdxCaller", function () {
  describe("Instancing Options", function () {
    const checkIdxClassState = (classInstance, versionToCheck = "/v1") => {
      ...
      assert.equal(
        classInstance.indexerUrl,
        "https://index-frontend-sg4efdgo3q-uc.a.run.app"
      );
      assert.equal(
        classInstance.axiosHandler.endpointBaseURL,
        "https://index-frontend-sg4efdgo3q-uc.a.run.app" + versionToCheck
      );
    };
    ...
    it("Should show expected class state when instanced w/ config object w/ version", function () {
      let idxCallerObj2 = new IdxCaller({
        indexerURL: testIdxEndpoint,
        indexerVersion: 2,
      });
